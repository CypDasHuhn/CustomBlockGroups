package dev.cypdashuhn.cbg.commands

import dev.cypdashuhn.cbg.database.GroupManager
import dev.cypdashuhn.cbg.globalAsDefault
import dev.cypdashuhn.rooster.common.util.tSend
import dev.jorel.commandapi.arguments.CustomArgument
import org.bukkit.Bukkit
import org.bukkit.World
import org.bukkit.command.CommandSender
import org.bukkit.entity.Player

fun defaultWorlds(sender: CommandSender): List<World> {
    return if (globalAsDefault) Bukkit.getWorlds()
    else listOf((sender as Player).world)
}

fun getWorldNames(worlds: List<World>?, sender: CommandSender): List<String> {
    return (worlds ?: defaultWorlds(sender)).map { it.name }
}

fun hasGroupsInScope(sender: CommandSender): Boolean {
    val worldNames = defaultWorlds(sender).map { it.name }
    return GroupManager.getGroups(worldNames).isNotEmpty()
}

fun validateFileName(name: String, sender: CommandSender): String {
    val invalidChars = listOf('\\', '/', ':', '*', '?', '"', '<', '>', '|')
    val reservedNames = listOf(
        "CON",
        "PRN",
        "AUX",
        "NUL",
        "COM1",
        "COM2",
        "COM3",
        "COM4",
        "COM5",
        "COM6",
        "COM7",
        "COM8",
        "COM9",
        "LPT1",
        "LPT2",
        "LPT3",
        "LPT4",
        "LPT5",
        "LPT6",
        "LPT7",
        "LPT8",
        "LPT9"
    )

    if (name in reservedNames) {
        sender.tSend("cbg.error.reserved_name", "name" to name)
        throw CustomArgument.CustomArgumentException.fromMessageBuilder(
            CustomArgument.MessageBuilder("Reserved name not allowed: ").appendArgInput()
        )
    }

    if (name.endsWith('.')) {
        sender.tSend("cbg.error.name_ends_with_dot", "name" to name)
        throw CustomArgument.CustomArgumentException.fromMessageBuilder(
            CustomArgument.MessageBuilder("Name cannot end with a dot: ").appendArgInput()
        )
    }

    name.forEach { char ->
        if (char in invalidChars) {
            sender.tSend("cbg.error.invalid_character", "char" to char.toString())
            throw CustomArgument.CustomArgumentException.fromMessageBuilder(
                CustomArgument.MessageBuilder("Invalid character '$char' in name: ").appendArgInput()
            )
        }
    }

    return name
}
